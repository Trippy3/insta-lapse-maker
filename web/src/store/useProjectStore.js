...
export const useProjectStore = create((set, get) => ({
    ...
    moveClip: (id, delta) => {
        ...
        if (idx < 0 || newIdx < 0 || newIdx >= sorted.length)
            return;
    ...
    updateClipDuration: (id, duration) => {
        const p = get().project;
        const clips = p.clips.map((c) => c.id === id ? { ...c, duration_s: duration } : c);
    ...
    setTransition: (afterClipId, kind, durationS) => {
        ...
        let transitions;
        if (existing) {
            transitions = p.transitions.map((t) => t.after_clip_id === afterClipId ? { ...t, kind, duration_s: durationS } : t);
        }
        else {
